...
export default function DatePicker({ value, onChange, label, error }) {
  ...
  return (
    <div className="flex flex-col gap-1.5 relative" ref={dropdownRef}>
      ...
      <div 
        className={`w-full bg-surface/50 border rounded-xl2 px-4 py-3 flex items-center justify-between cursor-pointer transition-all
          ${isOpen ? 'border-primary ring-2 ring-primary/20' : 'border-sage/30 hover:border-primary/50'}
        `}
        onClick={() => setIsOpen(!isOpen)}
      >
        <div className="flex items-center gap-3">
          <CalendarIcon size={18} className={value ? 'text-primary' : 'text-text-secondary'} />
          <span className={`font-medium ${value ? 'text-text-primary' : 'text-text-secondary'}`}>
            {value ? format(selectedDate, 'EEEE, d MMMM yyyy', { locale: idLocale }) : 'Pilih Tanggal'}
          </span>
        </div>
      </div>

      {isOpen && (
        <div className="absolute bottom-full left-0 right-0 mb-2 bg-surface rounded-xl2 shadow-[0_-8px_30px_rgba(85,117,97,0.15)] border border-sage/10 p-3 z-50 origin-bottom animate-in fade-in zoom-in-95">
          {/* Header */}
